...
namespace JwtChallenge.Controllers
{
    public class HomeController : Controller
    {
        ...
        private string GenerateJWT(string user)
        {
            ...
            var jwtPassword = _config.GetValue<string>("JWTPass", "DEFAULT_KEY_THAT_IS_VERY_LONG");
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(new Claim[] 
                {
                    new Claim("sub", user),
                    new Claim("iss", "https://sso.binsec.cloud")
                }),
                Expires = DateTime.UtcNow.AddDays(7),
                SigningCredentials = new SigningCredentials(new SymmetricSecurityKey(Encoding.ASCII.GetBytes(jwtPassword)), SecurityAlgorithms.HmacSha256Signature)
            };
